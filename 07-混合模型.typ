...
其中，$w_i$是每个组分（或类）的权重。可将$w_i$解释为组分$i$的概率，因此其值被限制在$[0, 1]$区间内，且$sum_(i=1)^k w_i = 1$。组分$p_i (y|θ_i)$几乎可以是任何分布，如 Gaussian 或 Poisson，甚至更复杂的对象，如分层模型或神经网络。为了拟合一个有限混合模型，需要提供一个$K$的值，$K$的选取基于事先知道的真值，或是经验上的猜测。
...
$ f(x) = frac(1, "Bin"(𝜶)) product_(i=1)^k x_i^(𝜶_i - 1) $

其中，$"Bin"(𝜶) = frac(product_(i=1)^k Γ(𝜶_i), Γ(sum_(i=1)^k 𝜶_i))$，$𝜶 = (α_1, …, α_k)$称为浓度向量。
...
$ sum_(k=1)^∞ w_k ⋅ δ_(θ_k)(θ) = f(θ) ∼ D P (α, H) $
...
$ w_k = β_k^′ ⋅ product_(i=1)^(k - 1)(1 - β_i^′) $

其中

- $w_k$是一个子棍的长度
- $product_(i=1)^(k - 1)(1 - β_i^′)$是剩余部分的长度
